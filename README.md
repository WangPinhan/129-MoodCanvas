...
| **解忧杂货铺（工具包）** | 提供呼吸练习、想法重构、安慰语卡片等轻量调节工具（非医疗用途），增强陪伴感。 |
...
> *（此处可放置 GIF 或截图，展示输入页、动态画布、时间长廊、周报、树洞等）*
...
- （可选）用于登录与树洞的云端服务（默认地址 `http://127.0.0.1:8765`）
...
| 类别 | 外部参考 | 小组自研 |
|------|----------|----------|
| 情绪分析 | OpenAI/Hugging Face 基础 API | 设计中文情绪分析 Prompt、结构化 JSON Schema、异常处理与多轮调参 |
| 可视化 | QPainter/Canvas 基础示例 | 情绪向量 → 颜色/动画参数的自定义映射规则、动态极光与粒子系统 |
| UI/UX | 通用图标库、组件库 | 完整治愈系视觉语言（低饱和渐变、圆角卡片、星球 IP）、交互流程与周报排版 |
| 数据存储 | SQLite/LocalStorage 基础用法 | 本地数据结构设计、按日/周/月聚合统计算法、隐私策略说明 |
| 疗愈文案 | 公开心理教育资源 | 非诊断化的文案改写、ABC 卡片交互、工具箱任务设计与风险提示 |
...
感谢 Qt 开源社区、大模型 API 服务商，以及所有参与 MoodCanvasQt 设计、开发与测试的小组成员。

---

**心象日记 · 让每一种情绪，都被温柔地看见。**  
欢迎 Star ⭐ 或 Fork 体验，也欢迎提出建议与反馈！
